...
Reglas:
- Agrupa por TEMA, no por documento. Cada fragmento relevante va en el núcleo que mejor le corresponda.
- Ignora fragmentos irrelevantes al encargo (no los fuerces a un núcleo).
- Entre 3 y 6 núcleos.
- NO escribas nada fuera del JSON.`;

const CLAIMS_SYSTEM = `Eres un investigador cotejando fuentes sobre un núcleo temático de historia de Colombia/América Latina. Recibes fragmentos numerados [N], cada uno de un documento. Destila AFIRMACIONES atómicas y verificables relevantes al núcleo, y mapea qué fragmentos las respaldan, complementan o contradicen.
...
Reglas:
- "fuentes": números [N] de ESTE bloque que respaldan la afirmación. NUNCA inventes números.
...
- Entre 4 y 12 afirmaciones por núcleo. Densidad > volumen.
- NO escribas nada fuera del JSON.`;
...
async function identifyCores(
  ...
  const context = buildContextBlock(chunks); // numera [1..N]
  ...
  const list = Array.isArray(raw.nucleos) ? raw.nucleos.slice(0, MAX_NUCLEOS) : [];
  list.forEach((n, i) => {
...
export async function triangular(
  ...
  const nucleos = await identifyCores(chunks, brief);
  ...
  const settled = await Promise.allSettled(
    nucleos.map((core, i) => distillClaims(core, chunkMap, i * 1000 + 1))
  );
